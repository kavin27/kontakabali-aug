...
class Auth_model extends CI_Model
{
    	...
    	if($username!='' && $password!=''){
	    	...
	    	if($result){
                if($result->confirmed){
                    ...
                    $output = array('status' =>'ERROR' , 'message'=>'User activation mail is not confirmed ' );
                }
                
	    	}
	    	else{
	    		$output = array('status' =>'ERROR', 'message'=>'Username and password does not match');
	    	}
    	}
    	else{
    ...
    public function forgotPassword($data){
        if($data['email'] !='' ){
            ...
            if($check_exist){
                ...
                $output = array('status' => 'ERROR' , 'message' => 'This is not Registred User' );
            }
        }
        return $output;
    }
    ...
    public function initInvite($id){
        return $this->db->where('userId', $id)->count_all_results('invite');
    }
    ...
    public function validateInvite($data){
        return $this->db->where('email', $data['email'])->where('inviteKey', $data['key'])->count_all_results('invite');
    }
}
