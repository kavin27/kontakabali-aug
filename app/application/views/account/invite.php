...
if(!isset($_REQUEST) || empty($_REQUEST) || !isset($_REQUEST['email']) || !isset($_REQUEST['key'])){
	redirect("");
}
...
curl_setopt($ch, CURLOPT_URL, base_url() . "api/auth/validateInvite");
	...
	<div class="body welcome_process">
			...
			<div role="main" class="main">
				...
				<div class="container">
					...
					<p>Welcome to It’s Over Easy<br>You have been invited by your spouse to participate in the divorce process. <br>
Please create an account to continue.</p>
						<div class="col-md-12 login_form">	
							<form name="form_1" id="addSpouse" action="../../api/auth/addSpouse" method="post">
								<input type="hidden" name="key" value="<?php echo $keyval; ?>">
								<input class="input_field" name="email" placeholder="User Name" type="email" value="<?php echo $email; ?>" readonly>
								<input class="input_field" id="password" name="password" placeholder="Password" type="password" >
								<input class="input_field" name="repassword" placeholder="Confirm Password" type="password" >
								<input class="ioe_btn new" name="continue" value="Create New" type="submit" >
							</form>
						</div>
					</div>
				</div>
